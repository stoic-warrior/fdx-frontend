...
import { Plus, Edit2, Trash2, X, Check, ChevronDown, ChevronUp, Target, Activity, Flag } from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            {showNewWigForm && (
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border-2 border-blue-200 shadow-lg">
                    ...
                    {/* 1단계: 타입 선택 */}
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 mb-2">목표 타입 선택</label>
                        <div className="flex gap-4">
                            <button
                                type="button"
                                onClick={() => setNewWig({...newWig, measureType: 'NUMERIC', fromX: '', toY: '', unit: ''})}
                            ...
                            >
                                ...
                                <div className="text-sm text-gray-500">숫자로 측정 (예: 체중 80→70kg)</div>
                            </button>
                            <button
                                type="button"
                                onClick={() => setNewWig({...newWig, measureType: 'STATE', fromX: '', toY: '', unit: ''})}
                                className={`flex-1 p-4 rounded-lg border-2 transition-all ${
                                    newWig.measureType === 'STATE'
                                        ? 'border-purple-500 bg-purple-50 text-purple-700'
                                        : 'border-gray-200 hover:border-gray-300'
                                }`}
                            >
                                <div className="font-bold">상태형</div>
                                <div className="text-sm text-gray-500">마일스톤으로 측정 (예: 백수→취업)</div>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="md:col-span-2">
                            ...
                            <input
                                type="text"
                                placeholder={newWig.measureType === 'NUMERIC' ? "예: 체중 감량" : "예: 백엔드 개발자 취업"}
                                value={newWig.title}
                        ...
                        {newWig.measureType === 'NUMERIC' ? (
                            <>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">시작값 (숫자)</label>
                                    <input
                                        type="number"
                                        step="0.1"
                                        placeholder="예: 80"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">목표값 (숫자)</label>
                                    <input
                                        type="number"
                                        step="0.1"
                                        placeholder="예: 70"
                            ...
                            <>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">시작 상태</label>
                                    <input
                                        type="text"
                                        placeholder="예: 백수, 초보자"
                                        value={newWig.fromX}
                                        onChange={e => setNewWig({...newWig, fromX: e.target.value})}
                                        className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
                                    />
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">목표 상태</label>
                                    <input
                                        type="text"
                                        placeholder="예: 취업 성공, 전문가"
                                        value={newWig.toY}
                                        onChange={e => setNewWig({...newWig, toY: e.target.value})}
                                        className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500"
                                    />
                        ...
                        <div className={newWig.measureType === 'NUMERIC' ? '' : 'md:col-span-2'}>
                    ...
                    <div className="flex justify-end gap-3 mt-4">
                        ...
                        <button
                            ...
                            disabled={!newWig.title || !newWig.fromX || !newWig.toY || !newWig.byWhen || (newWig.measureType === 'NUMERIC' && !newWig.unit)}
                                                        ...
                                                        {editingMilestone?.id === milestone.id ? (
                                                            ...
                                                            <>
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <span className={milestone.completed ? 'text-gray-400' : 'text-gray-700'}>
                                                                        {milestone.name}
                                                                    </span>
                                                                    {milestone.completed && (
                                                                        <span className="text-xs text-green-600 bg-green-100 px-2 py-0.5 rounded">완료</span>
                                                                    )}
                                                                </div>
                                                                <div className="flex gap-1">
                                                                    <button
